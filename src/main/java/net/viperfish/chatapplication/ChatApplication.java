...
public class ChatApplication extends WebSocketApplication {
     ...
     *
     * @param type the type of {@link LSRequest} that the handler can handles
     ...
     * @param socket the originating websocket
     * @return a {@link LSRequest} that represents the incoming JSON packet
     * @throws JsonParseException if the incoming websocket packet is not proper
     * JSON
     * @throws JsonMappingException if the incoming websocket packet fails to
     * map to a {@link LSRequest}
     */
     ...
     *
     * @param payload the payload to sent if there is a target
     * @param status the status to change if the target is offline
     * @throws JsonGenerationException if failed to generate JSON payload from
     * {@link LSPayload}
     * @throws JsonMappingException if failed to map values in {@link LSPayload}
     * to JSON fields
     */
    private boolean sendPayload(LSPayload payload)
     ...
     * handler. However, if no handler is found, the status is 202 No Handler.
     * @throws JsonGenerationException if failed to generate JSON for the
     * {@link LSPayload}
     * @throws JsonMappingException if failed to map {@link LSPayload} fields
     * into JSON fields
     */
    private LSResponse handleRequest(LSRequest req) throws JsonGenerationException, JsonMappingException {
        ...
        if (handler != null) {
            ...
            List<String> failedTargets = new LinkedList<>();
            // send the payload to another client if any
            for (LSPayload payload : resps) {
                if(!sendPayload(payload)) {
                    failedTargets.add(payload.getTarget());
                }
            }
            if(!failedTargets.isEmpty()) {
                status.setStatus(LSResponse.USER_OFFLINE, Arrays.toString(failedTargets.toArray()));
            }
        } else {
     ...
     * {@link LSRequest}.
     *
     * @param statusPayload the {@link LSPayload} that will contain the
     * {@link LSResponse} data
     * @param status the result of the handler to the origin client
     * @param origin the websocket of the source client
     * @throws JsonGenerationException if failed to generate JSON from the
     * {@link LSPayload}
     * @throws JsonMappingException if failed to map {@link LSPayload} fields
     * into JSON fields.
     */
     ...
     * clients.
     *
     * @param socket the originating socket of the request
     * @param text the string body of the websocket packet
     */
    @Override
